...
export default {
  ...
  methods: {
    ...
    selectItem (option, value) {
      // this.$refs.myinput.placeholder = option.Date
      this.current = option.Date
    ...
    //   if(this.$route.fullPath ==='/' || this.$route.fullPath.includes('movie-search') || this.$route.fullPath.includes('-y-') || this.$route.fullPath.includes('sehirler') || this.$route.fullPath.includes('universiteler') ) {
        if(option.Date != 'No results!') {
         this.search = option.Date
       }
     //  }
    ...
    getFiltered (options) {
        return options.filter(item => {
          if(item.Date) {
          return item.Date.toLocaleUpperCase().indexOf(this.search.toLocaleUpperCase()) !== -1
          } else return ''
        })
    },    
  },
  ...
  created() {
          if (!window.localStorage.getItem(this.Date) || window.localStorage.getItem(this.Date) === '' || window.localStorage.getItem(this.Date) === 'null') {
              this.selected = false
              this.search = ''
              
          } else {
              this.selected = true
              this.search = window.localStorage.getItem(this.Date)
          } 
  },
    watch: {
    $route () {
